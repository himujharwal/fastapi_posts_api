...
# def login(user_credential:User, db : Session = Depends(database.get_db)):
...
def login(user_credential: OAuth2PasswordRequestForm = Depends(), db : Session = Depends(database.get_db)):
    ...
    user = db.query(models.User).filter(models.User.email == user_credential.username).first()  # (username) becuase that how the Oauth2form data use it as 
